...
import com.aio.portable.swiss.structure.database.jpa.annotation.order.Order;
...
public abstract class PageableSort {
    ...
    public final static Sort buildSort(Class<?> clazz) {
        List<Field> fieldList = BeanSugar.Fields.getDeclaredFieldIncludeParents(clazz).stream().filter(c -> c.isAnnotationPresent(Order.class)).collect(Collectors.toList());
