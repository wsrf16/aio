...
import com.aio.portable.swiss.suite.bean.type.KeyValueHolder;
...
public class RestTemplater {
    public final static class Headers {
        ...
        public static KeyValueHolder<String, String> buildBasicAuthorizationHeaderPair(String username, String password) {
            String plainCreds = MessageFormat.format("{0}:{1}", username, password);
            String token = JDKBase64Convert.encodeToString(plainCreds);
            KeyValueHolder<String, String> entry = new KeyValueHolder<>(HttpHeaders.AUTHORIZATION, "Basic " + token);
            return entry;
        }

        public static KeyValueHolder<String, String> buildBearerAuthorizationHeaderPair(String token) {
            KeyValueHolder<String, String> entry = new KeyValueHolder<>(HttpHeaders.AUTHORIZATION, "Bearer " + token);
            return entry;
        }
    }
