...
public abstract class StreamSugar {
    ...
    public static <T> Stream<T> except(final Stream<T> stream1, final Stream<T> stream2) {
     ...
     */
    public static <T> Collection<T> toLazyCollection(final Stream<T> stream) {
        return toLazyCollection(stream.iterator());
    }

    public static <T> Collection<T> toLazyCollection(final Iterator<T> iterator) {
     ...
     */
    public static <T> Collection<T> toConcurrentLazyCollection(final Stream<T> stream) {
        return toConcurrentLazyCollection(stream.iterator());
    }

    public static <T> Collection<T> toConcurrentLazyCollection(final Iterator<T> iterator) {
        return toLazyCollection(iterator, true);
    }

    private static <T> Collection<T> toLazyCollection(final Iterator<T> iterator, final boolean concurrent) {
        return createLazyCollection(iterator, concurrent);

    }

    private static <T> Collection<T> createLazyCollection(final Iterator<T> iterator, final boolean concurrent) {
...
//    public static <T, K, U> Collector<T, ?, Map<K, U>> toMap(Function<? super T, ? extends K> keyMapper, Function<? super T, ? extends U> valueMapper, K replaced) {
...
//    public static  <T, K, U> Collector<T, ?, Map<K,U>> toMap(Function<? super T, ? extends K> keyMapper, Function<? super T, ? extends U> valueMapper, BinaryOperator<U> mergeFunction, K replaced) {
//        Function<? super T, ? extends K> newKeyMapper = (k -> k == null ? replaced : keyMapper.apply(k));
...
//    }
    ...
    public static <T, K, V> HashMap<K, V> toMap(Stream<T> stream, Function<T, K> keyMapper, Function<T, V> valueMapper) {
