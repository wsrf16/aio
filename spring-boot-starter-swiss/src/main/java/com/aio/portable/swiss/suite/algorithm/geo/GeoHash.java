...
public final class GeoHash implements Comparable<GeoHash>, Serializable {
     ...
     */
    public static GeoHash withCharacterPrecision(double latitude, double longitude, int precision) {
        if (precision > MAX_CHARACTER_PRECISION) {
            throw new IllegalArgumentException("A geohash can only be " + MAX_CHARACTER_PRECISION + " character long.");
        }
        int desiredPrecision = (precision * 5 <= 60) ? precision * 5 : 60;
...
//    }
    ...
    public static GeoPoint decode(String geoHash) {
        return decodeToGeoHash(geoHash).getPoint();
    }
     ...
     */
    public static GeoHash decodeToGeoHash(String geoHash) {
    ...
    public static String encode(double latitude, double longitude, int precision) {
        GeoHash hash = withCharacterPrecision(latitude, longitude, precision);
    ...
    private static void setBoundingBox(GeoHash hash, double[] latitudeRange, double[] longitudeRange) {
        hash.boundingBox = new BoundingBox(new GeoPoint(latitudeRange[0], longitudeRange[0]), new GeoPoint(
                latitudeRange[1],
                longitudeRange[1]));
    }
    ...
    public GeoPoint getBoundingBoxCenterPoint() {
