 ...
 */
public class LoggerHubImp implements LoggerHub {
    List<LoggerRoot> loggers;
    ...
    public LoggerHubImp(LoggerRoot logger) {
        loggers = new ArrayList<>();
        addRegister(logger);
    }

    public LoggerHubImp(List<LoggerRoot> loggers) {
        this.loggers = loggers;
    }

    public void addRegister(LoggerRoot logger) {
        loggers.add(logger);
    }

    public static LoggerHubImp build(LoggerRoot... logger) {
        List<LoggerRoot> loggers = logger == null ? Collections.emptyList() : new ArrayList<>(Arrays.asList(logger));
        return new LoggerHubImp(loggers);
    }

//    public static LoggerHubImp buildAsync(LoggerRoot... logger) {
//        LoggerHubImp loggerHub = build(logger);
//        loggerHub.setAsync(true);
//        return loggerHub;
//    }

//    public static LoggerHubImp buildLog4j2(Class clazz){
...
//    }
//
//    public static LoggerHubImp buildLogback(Class clazz){
//        LoggerHubImp logger = LoggerHubImp.build();
//        logger.addRegister(ConsoleLogger.build(clazz));
...
//    }
//
//    public static LoggerHubImp buildFile(Class clazz){
//        LoggerHubImp logger = LoggerHubImp.build();
//        logger.addRegister(ConsoleLogger.build(clazz));
...
//    }
    ...
    protected static void throwProviderEmpty() {
        try {
            throw new IllegalArgumentException(MessageFormat.format("{0} is Empty.Please register and try again!", LoggerHubImp.class.getTypeName()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
