package com.york.portable.swiss.distribute;
...
//            List<String> children = fullChildren(zookeeper, path);
    ...
    public static boolean exists(ZooKeeper zookeeper, String path) throws KeeperException, InterruptedException {
    ...
    public static String createIfNotExists(ZooKeeper zookeeper, String path) throws KeeperException, InterruptedException {
        return exists(zookeeper, path) ? "" : zookeeper.create(path, null, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
    }

    public static String createIfNotExists(ZooKeeper zookeeper, String path, byte[] bytes) throws KeeperException, InterruptedException {
        return exists(zookeeper, path) ? "" : zookeeper.create(path, bytes, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
    }

    public static String createEphemeralIfNotExists(ZooKeeper zookeeper, String path) throws KeeperException, InterruptedException {
        return exists(zookeeper, path) ? "" : createEphemeral(zookeeper, path);
    }

    public static String createEphemeralIfNotExists(ZooKeeper zookeeper, String path, byte[] bytes) throws KeeperException, InterruptedException {
        return exists(zookeeper, path) ? "" : createEphemeral(zookeeper, path, bytes);
    }
