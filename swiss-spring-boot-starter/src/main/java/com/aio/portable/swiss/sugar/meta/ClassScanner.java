...
class ClassScanner implements ResourceLoaderAware {
    ...
    public static List<Class> scan(String[] basePackages,
        ...
        if(!CollectionSugar.isEmpty(annotations)) {
            for (Class<? extends Annotation> anno : annotations) {
                scanner.addIncludeFilter(new AnnotationTypeFilter(anno));
            }
        }

        List<Class> classes = new ArrayList<>();
        for (String s : basePackages)
            classes.addAll(scanner.doScan(s));
        return classes;
    }
    ...
    private static void main(String[] args) {
        ClassScanner.scan("com.hjzgg.test.thirdjar.config,com.hjzgg.test.thirdjar.web", (Class<? extends Annotation>)null)
